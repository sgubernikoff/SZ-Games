...
function Roulette({user, setUser}) {
  ...
  function makeBet(e) {
    if (e.target.className.includes("clickable") || e.target.parentNode.className.includes("clickable")) {
  ...
  function boardElement(cls, text, name) {
    return (
    <div className={Object.keys(bets).includes(name) ? `${cls} no-text`: cls} name={name}>
      {text}
      {
        Object.keys(bets).includes(name) ? <img onClick={removeBet} src={chips.find(chip => parseInt(chip.value) === bets[name]).image} alt="chip" className="table-chip"/> : null
      }
    </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="chip-container">
        {
          chips.map(chip => {
            return <div key={uuid()}><img onClick={getChip} name={chip.value} src={chip.image} alt="chip" className={currentChip === chip.value ? "current-chip" : "chip"}/></div>
          })
        }
      </div>
